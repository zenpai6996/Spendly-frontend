...
const Input = ({ value, onChange, placeholder, label, type }) => {
 ...
 return (
    <div className="mb-1">
      <label className="text-sm text-slate-800 dark:text-gray-300 block mb-2">
        {label}
      </label>
      <div className={`w-full flex justify-between items-center gap-3 text-sm text-black bg-slate-100 rounded px-4 py-3 border-2 ${
        ...
        {type === "password" && (
          <div onClick={toggleShowPassword} className=" cursor-pointer">
            {showPassword ? (
              <svg width="22"  height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-gray-400 hover:text-primary transition-colors duration-200 ease-in-out">
